...
class NavigationManager(
    ...
    var focused by mutableStateOf(FocusPosition(section = Section.DEVICES, row = 0, col = 0))
        private set
    ...
    private fun moveUp() {
        focused = when (focused.section) {
            Section.BOTTOM_BAR -> FocusPosition(
                section = currentSectionFromDestination(),
                row = maxGridRow(),
                col = min(focused.col, lastColumnInRow(maxGridRow()))
            )
            Section.DEVICES,
            Section.ROUTINES -> {
                if (focused.row > 0) {
                    FocusPosition(
                        ...
                        row = focused.row - 1,
                        col = min(focused.col, lastColumnInRow(focused.row - 1))
                    )
                } else {
                    focused
                }
            ...
            Section.ROUTINES -> {
                ...
                if (nextRow <= maxGridRow()) {
                    FocusPosition(
                        section = focused.section,
                        row = nextRow,
                        col = min(focused.col, lastColumnInRow(nextRow))
                    )
                } else {
                    FocusPosition(
                        section = Section.BOTTOM_BAR,
                        row = 0,
                        col = min(focused.col, bottomBarItems - 1)
                    )
                }
            }
        }
    ...
    private fun moveLeft() {
        focused = focused.copy(col = (focused.col - 1).coerceAtLeast(0))
    }
    ...
    private fun moveRight() {
        ...
        focused = focused.copy(col = min(focused.col + 1, maxCol))
    }
    ...
    private fun cancelSelection() {
        if (focused.section == Section.BOTTOM_BAR) {
            focused = FocusPosition(Section.DEVICES, 0, 0)
        }
    }
    ...
    private fun confirmSelection() {
        when (focused.section) {
            ...
            Section.DEVICES  -> { /* YES does nothing on devices  */ }
            Section.ROUTINES -> { /* YES does nothing on routines */ }
        }
    }
    ...
    private fun turnOnFocused() {
        ...
        if (devices[index].isSensible) {
            pendingConfirmation = PendingConfirmation.TurnOnDevice(index)
            return
        }
        turnOnDevice(index)
    }
    ...
    private fun turnOffFocused() {
        ...
        val index = focusedDeviceIndex() ?: return
        if (devices[index].isSensible) {
            pendingConfirmation = PendingConfirmation.TurnOffDevice(index)
            return
        }
        turnOffDevice(index)
    }
    ...
    fun toggleDevice(index: Int) {
        devices = devices.mapIndexed { i, d -> if (i == index) d.copy(isOn = !d.isOn) else d }
    }

    fun turnOnDevice(index: Int) {
        devices = devices.mapIndexed { i, d -> if (i == index) d.copy(isOn = true)  else d }
    }

    fun turnOffDevice(index: Int) {
        devices = devices.mapIndexed { i, d -> if (i == index) d.copy(isOn = false) else d }
    }
    ...
    private fun stopRoutine(index: Int) {
        routines = routines.mapIndexed { i, r -> if (i == index) r.copy(isOn = false) else r }
    }
    ...
    private fun confirmPending() {
        when (val action = pendingConfirmation) {
            is PendingConfirmation.TurnOnDevice  -> turnOnDevice(action.index)
            is PendingConfirmation.TurnOffDevice -> turnOffDevice(action.index)
            null -> {}
        }
    ...
    private fun maxGridRow(): Int {
        ...
        return if (count == 0) 0 else (count - 1) / columns
    }

    private fun lastColumnInRow(row: Int): Int {
        val count = currentGridCount()
        val remaining = count - row * columns
        return when {
            remaining <= 0      -> 0
    ...
    private fun currentGridCount(): Int = when (focused.section) {
