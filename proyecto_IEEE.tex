...
El preprocesamiento se implement\'o en Kotlin de forma que replique el contrato de entrenamiento: captura de audio mono PCM a 16\,kHz en ventanas de 1\,s, c\'alculo de log mel-espectrograma, reescalado bilineal a $128\times128$, normalizaci\'on y empaquetado del tensor NCHW \texttt{(1,1,128,128)} antes de la invocaci\'on del modelo. Este contrato est\'a documentado en el c\'odigo y se mantuvo consistente con el pipeline usado en el cuaderno de entrenamiento para minimizar discrepancias entre entrenamiento e inferencia.

\subsection{Coherencia salida--acciones en la interfaz}
La salida del clasificador es un vector de diez logits; el \'indice del m\'aximo se mapea a la etiqueta l\'exica y de ah\'i a acciones de navegaci\'on o de control del hogar simulado (p.\,ej.\ mover foco con \texttt{up/down/left/right}, confirmar con \texttt{yes/no}, conmutar dispositivos con \texttt{on/off} e iniciar/detener rutinas con \texttt{go/stop}). La aplicaci\'on est\'a implementada en Jetpack Compose \cite{jetpack_compose} como experiencia navegable; los comandos se generan a partir de inferencia local sobre el micr\'ofono y se despachan a un manejador de navegaci\'on/estado que traduce cada comando a cambios de foco, navegaci\'on entre pantallas y acciones sobre dispositivos o rutinas.
